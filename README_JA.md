...
バージョン：v1.0.0。初回導入は[日英README](README.md)と[Windows/WSL別の依存導入手順](docs/DEPENDENCIES.md)を順番に進めてください。ここでは画面内の操作を詳しく説明します。

生成実績は元配布構成の記録です。公開用の依存分離後にGPU生成を再実行したものではありません。今回の静的・ZIP検査と未確認範囲は[VALIDATION.md](VALIDATION.md)で区別しています。

映像4ステップ Fused＋SLA／音声再精錬2ステップ／可変尺／日本語LM Studio変換・完成英語直接入力。

## 同梱内容

- `workflows/`：`T2V_4step.json`、`I2V_4step.json`、`Ref2V_4step.json`の3種類。サンプル日本語だけを入力済み。各ワークフロー内にも詳しい説明があります。
- `custom_nodes/`：自作の`comfyui-h3-standard-prompt`を収録。外部4パッケージは固定版を別途取得し、`patches/`の互換差分を適用します。
...
ZIPを展開し、自作ノード`comfyui-h3-standard-prompt`だけをComfyUIの`custom_nodes/`へコピーします。外部4パッケージは[詳細な導入手順](docs/DEPENDENCIES.md)に従い、固定コミットを取得して同梱パッチを適用してください。同名ノードを二重に導入せず、既存版は`custom_nodes/`の外へ退避します。Managerで一括更新すると互換修正が失われる場合があります。
...
## 導入
1. [LM Studio公式サイト](https://lmstudio.ai/)からインストールします。この配布ノードは `/api/v1/chat` に対応するLM Studioを使用します。
...
接続・モデル名を変える場所はプロンプト枠の小さな設定欄です。出力上限4096は「文字数」ではなくトークン数です。変換待ち時間は360秒。文章が途中で切れたら上限、時間切れならモデルの負荷と待ち時間を確認します。
...
秒数×24fpsから出力フレーム数を計算します。内部フレーム数はH3の形式に合わせ、最終出力を指定尺に整えます。約15秒を超える動画は前区間末尾の映像・音声を引き継いで続きの区間を生成します。台詞の数で勝手に尺を伸ばす設定ではありません。
...
## 必須カスタムノード
ZIPの自作ノード1フォルダーをComfyUI/custom_nodes/へコピーします。外部4ノードはdocs/DEPENDENCIES.mdに従い固定版を取得し、同梱パッチを適用します。
...
## 困ったとき
- 赤い不足ノード：5フォルダーの位置・固定版とパッチ・依存ライブラリ・起動ログを確認。
