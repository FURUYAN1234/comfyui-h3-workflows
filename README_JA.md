...
- `workflows/`：`T2V_4step_20260909182009_v1.1.4.json`、`I2V_4step_20260909182009_v1.1.4.json`、`Ref2V_4step_20260909182009_v1.1.4.json`の3種類。サンプル日本語だけを入力済み。各ワークフロー内にも詳しい説明があります。
...
LM変換はJSON Schema付きのOpenAI互換APIを使用します。制作指示の台詞化、同じ台詞の重複、未変換の日本語説明、短すぎる時間展開を検査し、理由を付けて最大2回再試行します。旧「原文続行」をONにしても原文をそのままH3へ渡しません。見出し・言語タグの軽微な形式差は許容します。

行内の時間範囲と秒表記を区間分割に反映し、後半への冒頭の再投入を防ぎます。単一ショット指定を保持し、指定された消失後の結末を継続する指示を生成します。映像モデルの出力品質を保証するものではありません。

個人PCの起動スクリプト、バックアップ処理、ドライブ設定変更は含みません。モデル、APIキー、辞書、参照画像、検証動画も同梱しません。


## v1.1.4の追加修正

20秒までを一括生成し、15秒位置での分割を避けます。長尺の実際の区間境界をまたぐ文章は最大3回の範囲で修正を要求します。日本語台詞の言語タグ省略を補完し、継続区間に完了済みの音響イベントを再投入しないよう調整しました。特定の人物・場所・物語を条件にした分岐はありません。

検証範囲と残る制限は VALIDATION.md、タグからの再構築方法は REPRODUCE.md を参照してください。
