...
v1.1.4のZIPには必要な5カスタムノードフォルダーが同梱されているにもかかわらず、英語READMEの詳細手順に「自作ノードだけをコピーし、外部4ノードを別取得・パッチ適用」と残っていた矛盾を修正しました。v1.1.5では5フォルダーをそのままComfyUIの`custom_nodes/`へコピーします。ワークフローと変換ノードの挙動はv1.1.4から変更していません。
