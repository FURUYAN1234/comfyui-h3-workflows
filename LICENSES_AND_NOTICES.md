...
comfyui-h3-standard-promptの尺検査と日本語禁止指定の解析を修正し、回帰テストを追加しました。第三者パッケージ、モデル、ライセンス構成はv1.1.8から変更していません。

### v1.2.0（2026-09-22）

独自作成の `comfyui-mv-workflow` をMITで追加しました。YuE2等が出力する音源・歌詞・manifestと利用者が指定するキャラクター画像をローカルで処理します。配布ZIPに曲、歌詞、キャラクター画像、生成MV、Whisper重みは含みません。H3モデル・Whisper・FFmpeg・入力素材にはそれぞれの配布元および権利者の条件が別途適用されます。

### v1.2.1（2026-09-23）

独自作成の `comfyui-mv-workflow` にschema v2の複数人物処理を追加しました。第三者パッケージ、モデル、同梱ライセンス、入力素材の扱いはv1.2.0から変更していません。
